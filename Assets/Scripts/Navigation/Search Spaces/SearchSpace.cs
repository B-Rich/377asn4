...
public abstract class SearchSpace : MonoBehaviour
{
			...
			if (value != showingNodeMarkers)
			{
				if (Graph != null)
				{
					foreach (Node node in Graph.Nodes)
					{
						if (node.gameObject != null)
						{
							node.gameObject.renderer.enabled = value;
						}
					}
				}
				...
				if (Graph != null)
				{
					foreach (LinkedList<Edge> edgeList in Graph.Edges)
					{
						foreach (Edge edge in edgeList)
						{
							if (edge.gameObject != null)
							{
								edge.gameObject.renderer.enabled = value;
							}
						}
					}
				}
	...
	protected void AddNodeObject(Node node, Vector2 nodePosition)
	{
		...
		node.gameObject.renderer.enabled = ShowingNodeMarkers;
		node.gameObject.renderer.material = gameObject.renderer.material;
		node.gameObject.renderer.castShadows = false;
		Destroy(node.gameObject.collider);
	}
	...
	protected void AddEdgeObject(Edge edge, Vector2 startPoint, Vector2 endPoint)
	{
		...
		edge.gameObject.renderer.material = gameObject.renderer.material;
		edge.gameObject.renderer.castShadows = false;
		edge.gameObject.renderer.enabled = ShowingEdgeMarkers;
		Destroy(edge.gameObject.collider);
	}
}